...
variable "nv_install_script" {
  type = string
  default = "install-nv-vgpu-driver.sh"
}
...
build {
  ...
  provisioner "file" {
    source = "scripts/${var.nv_install_script}"
    destination = "/tmp/driver_install.sh"
  }

  provisioner "file" {
    source = "drivers/nvidia-linux-grid-510_510.85.02_amd64.deb"
    destination = "/tmp/nvidia-linux-grid-510_510.85.02_amd64.deb"
  }

  provisioner "shell" {
    inline = ["sudo chmod +x /tmp/driver_install.sh",  "sudo /tmp/driver_install.sh"]
  }
